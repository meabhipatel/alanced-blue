...
const ViewHiringDetail = () => {
    ...
    const handleAcceptProject = async () => {
        try {
            const response = await axios.put(`http://127.0.0.1:8000/freelance/projects/accept/${hire_id}`, {}, {
                headers: {
    ...
    const handleRejectProject = async () => {
        try {
            const response = await axios.put(`http://127.0.0.1:8000/freelance/projects/reject/${hire_id}`, {}, {
                headers: {
                    Authorization: `Bearer ${accessToken}`
                }
            });
