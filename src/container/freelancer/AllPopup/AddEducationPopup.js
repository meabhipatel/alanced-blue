...
const AddEducationPopup = ({ closeAddEducation }) => {
  ...
  useEffect(() => {
      if (freelancerselfprofile && freelancerselfprofile[0]) {
          setqualification(freelancerselfprofile[0].qualification);
      }
  ...
  return (
    ...
    <div className='mt-10'>
            ...
            <input type="text" className='border mt-2 mb-6 py-1.5 px-2 rounded-md w-full focus:border-lime-400 focus:outline-none focus:ring-1 focus:ring-lime-600' placeholder='Ex: B.Tech' onChange={e => setqualification(e.target.value)}/>
        ...
        <select className="border py-2 px-2 rounded-md focus:border-lime-400 focus:outline-none focus:ring-1 focus:ring-lime-600 flex-1 bg-white opacity-50">
            <option value="" disabled selected>To (or expected graduation year)</option>
            ...
            <Link to='' onClick={handleSave}><span class="inline-block text-sm px-4 py-[10px] bg-gradient-to-r from-[#00BF58] to-[#E3FF75] border rounded border-none text-white mr-3 font-semibold" >Add</span></Link>
            <div class="p-0.5 inline-block rounded bg-gradient-to-b from-[#00BF58] to-[#E3FF75]" onClick={closeAddEducation}>
