  ...
  return (
    ...
    <div className=' container-sm px-36'>
        ...
        <h1 className=' text-2xl font-cardo font-semibold text-left'>Additional details</h1>
            ...
            onChange={onChange} required  class="mt-3 w-full  px-3 py-2 border-2 rounded-lg text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:ring-blue-500 focus:border-blue-500  dark:focus:ring-blue-500 dark:focus:border-blue-500" rows='15'></textarea>
        </div>
        </div>
        ...
        <div className=' basis-3/12' >
            ...
            <button
            className={`h-10 w-52 text-white text-base font-semibold mt-5 rounded ${isButtonEnabled()? 'bg-gradient-to-r from-[#0909E9] to-[#00D4FF]': 'bg-gray-300'}`} onClick={handleButtonClick}disabled={!isButtonEnabled()}>
            Send Proposal
            </button>
        </div>
        <div class="p-0.5 mt-5 rounded bg-gradient-to-b from-[#0909E9] to-[#00D4FF]">
        <Link to='/jobs' onClick={() => window.scrollTo(0, 0)}><button class="px-2 py-1 bg-[#f8faf9] rounded-sm"><p class="bg-gradient-to-r from-primary to-danger bg-clip-text text-transparent font-bold text-sm py-[4px] px-[16px]">Cancel</p></button></Link>
        </div>
        </div>
    </div>
    {showDialog && (
