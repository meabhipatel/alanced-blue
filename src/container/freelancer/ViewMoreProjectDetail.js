...
useEffect(() => {
  ...
  return (
    ...
    <div className=' container-sm px-40'>
        ...
        {clickable ?  
        <div className='h-16 mt-4 bg-blue-100 pl-5 rounded-md pt-2 text-left'><LightbulbOutlinedIcon/><span className='ml-4'>You have already submitted a proposal for this project.</span></div>:  '' }
        ...
        <div className='h-16 mt-4 bg-blue-100 pl-5 rounded-md pt-2 text-left'><DoDisturbAltOutlinedIcon/><span className='ml-4'>This project is closed ,you can't add proposal now</span></div>:  '' }
           ...
           {skill}
                ...
                { accessToken ? (
                    ...
                    <Link to="/login"  onClick={() => window.scrollTo(0, 0)}><span class='px-12 py-[15px] lg:mt-0 bg-gradient-to-r from-[#0909E9] to-[#00D4FF] border rounded border-none text-white font-inter text-base font-normal'>Apply Now</span></Link>
                )}
                </div>
