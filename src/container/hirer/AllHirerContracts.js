...
{viewallhirercontracts.length > 0 ? (
        ...
        <div className=' flex flex-row'>
            ...
            <span class="inline-block text-sm px-6 py-[10px] bg-gradient-to-r from-[#0909E9] to-[#00D4FF] border rounded border-none text-white font-semibold cursor-pointer" onClick={() => openReview(contract.hire_id)}>Add Review</span>
    </div>
    {isReviewOpen[contract.hire_id] && <AddReviewPopup closeReview={()=>closeReview(contract.hire_id)} contract={contract}/>} 
        ...
        Sent: <span className='text-[#0365c0]'>{formatDateInput(contract.Sent_time)}</span></p>
            </div>
            <div className=' basis-2/12'>
            <p className='font-inter text-[14px] text-[#031136] font-normal text-left'>
