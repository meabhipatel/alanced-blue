...
const ViewProposalNewTab = () => {
  ...
  <div className="relative w-24 h-24">
                    <img src={"http://13.233.123.209:8000/"+bidData.bid.freelancer_profilepic} alt="Profile" className="rounded-full w-full h-full border border-gray-200" />
  ...
  <div className="flex items-center">
                <h1 className="font-cardo text-[24px] text-[#031136] font-normal mr-1">{bidData.bid.freelancer_Name}
            </h1>
  ...
  <Link to=''><span class="inline-block text-sm px-10 py-[10px] mt-4 lg:mt-0 bg-gradient-to-r from-[#00BF58] to-[#E3FF75] border rounded border-none text-white mr-4 font-semibold">Message</span></Link>
    ...
    <p className="text-[#031136] opacity-50 text-[14px] font-inter py-2">{bidData.bid.freelancer_Name} has applied to or been invited to your or your company's job {project.title}</p>
    </div>
...
<p className="text-[#031136] opacity-50 text-[14px] font-inter py-4">{bidData.bid.description}</p>
    ...
    <div class="w-full md:w-[70%] bg-[#FFFFFF] border border-gray-200 border-opacity-30 text-left">
        ...
        <div className="flex justify-between items-center">
    <h1 className="font-cardo text-2xl text-[#031136] font-normal">{bidData.bid.freelancer_category}</h1>
    <div>
...
<p className="text-[#031136] opacity-50 text-[14px] font-inter py-5">Hello, I am software developer working as a full stack Mean developer, I can do this work with good efficiency in less time, please let me know when we can connect and discuss further for this and other projects.</p>
  ...
  (() => {
    try {
      const skillsArray = JSON.parse(bidData.bid.Freelancer_skills.replace(/'/g, '"'));
