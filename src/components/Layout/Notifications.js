...
function groupByDate(notifications) {
  ...
  return (
   ...
   <h1 className="font-cardo text-[26px] text-[#031136] text-left font-normal p-3">Notifications</h1>
                ...
                {notifs.map(notif => (
                    <div key={notif.id} className={`border-b cursor-pointer p-5 hover:bg-[#F6FAFD] ${!notif.is_read ? 'bg-[#f4f8fc]' : 'bg-white'} relative group`} onClick={() => markAsReadClient(notif.id)}>
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center">
                                <p className='opacity-50 text-md mr-6'>{convertToFormattedTime(notif.timestamp)}</p>
                                <i 
                        ...
                        <p className='opacity-50 text-md pt-1'>{notif.message}</p>
                    </div>
                ))}
            </div>
        ))}
